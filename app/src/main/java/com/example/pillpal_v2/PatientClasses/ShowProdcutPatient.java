...
import com.example.pillpal_v2.Model.ProductPatient;
...
public class ShowProdcutPatient extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_show_prodcut_patient);
        storage = FirebaseStorage.getInstance();
        fAuth = FirebaseAuth.getInstance();
        user = fAuth.getCurrentUser();
        fStore = FirebaseFirestore.getInstance();
        Button showsuppatient=findViewById(R.id.showprodcutpatient);
        showsuppatient.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ShowProdcutPatient.this, PatientHomePage.class);
        ...
        recyclerView = findViewById(R.id.product_list_recycler_Patient);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        db = FirebaseFirestore.getInstance();
        list = new ArrayList<Medicine>();
        myAdapter = new MedicineAdapter(ShowProdcutPatient.this, list,"product");
        recyclerView.setAdapter(myAdapter);

        // Initialize the df variable
        df = fStore.collection("MT_ADDedProduct");
